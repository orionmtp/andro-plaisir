...
$sql="update bbf set solde=solde-'$montant' where id='$id'";
$result= mysqli_query($db,$sql) or die('Erreur SQL !'.$sql.'<br>'.mysqli_error($db));
$sql="update bbf set solde=solde+'$montant' where id='$id2'";
$result= mysqli_query($db,$sql) or die('Erreur SQL !'.$sql.'<br>'.mysqli_error($db));
$sql="insert into bbfbankop values (NULL,'$id','$montant','$id2')";
$result= mysqli_query($db,$sql) or die('Erreur SQL !'.$sql.'<br>'.mysqli_error($db));
...
transaction effectuée<br>';
        ...
        echo '<form action="../index.html" method=POST><input type="submit" value="quitter le serveur"></form>';
echo '</body>';
